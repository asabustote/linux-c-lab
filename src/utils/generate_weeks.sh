#!/usr/bin/env bash
# generate_phase.sh - Auto-generate weekly issue templates for linux-c-lab

###############################################################################
# 0. linux-c-lab root auto-detection
###############################################################################
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"

find_linux_c_lab_root() {
    local dir="$SCRIPT_DIR"
    while [[ "$dir" != "/" ]]; do
        if [[ "$(basename "$dir")" == "linux-c-lab" ]]; then
            echo "$dir"
            return 0
        fi
        dir="$(dirname "$dir")"
    done
    return 1
}

LAB_ROOT="$(find_linux_c_lab_root)"
if [[ -z "$LAB_ROOT" ]]; then
    echo "[ERROR] linux-c-lab root not found."
    exit 1
fi

ISSUES_DIR="$LAB_ROOT/issues"

###############################################################################
# 1. Roadmap (Phase 1 Example) — You can extend for Phase 2, 3, 4
###############################################################################
# Week definitions (example: Design Week → Week01, Blog Week → Week06)

declare -A GOALS
declare -A TODOS
declare -A REFERENCES
declare -A NEXT_WEEK

# Week01 (Design Week)
GOALS[1]="Linux-c-lab プロジェクト全体の設計週間"
TODOS[1]="- [ ] ディレクトリ構造の最終決定
- [ ] 必要となるサーバ構成の洗い出し
- [ ] 使用技術の一覧化
- [ ] Phase1〜Phase4の粒度調整"
REFERENCES[1]="- Linux FHS
- systemd ドキュメント
- Cプロジェクト構成 参考資料"
NEXT_WEEK[1]="Week02: Ubuntu環境構築 + 基本設定"

# Week02
GOALS[2]="Ubuntu仮想環境構築と初期設定"
TODOS[2]="- [ ] Multipass で VM 構築
- [ ] SSH鍵認証設定
- [ ] ufw の基本設定
- [ ] Neovim 導入と最低限の設定"
REFERENCES[2]="- Multipass Docs
- OpenSSH Docs
- Neovim Docs"
NEXT_WEEK[2]="Week03: systemd サービス理解 + ネットワーク基礎"

# Week03
GOALS[3]="systemd 基礎とネットワーク理解"
TODOS[3]="- [ ] systemd でサービス起動テスト
- [ ] basic target/wants の読解
- [ ] Ping/TCP/UDP の挙動確認
- [ ] C言語 で簡易TCPサーバ雛形を書く"
REFERENCES[3]="- systemd.unit
- Beej's Network Guide"
NEXT_WEEK[3]="Week04: C言語基礎 + TCPサーバ実装"

# Week04
GOALS[4]="C言語基礎とTCPサーバ実装の本格化"
TODOS[4]="- [ ] ポインタのおさらい
- [ ] ループ・メモリ管理強化
- [ ] TCPサーバ v1 を完成させる
- [ ] ログ出力基盤の作成"
REFERENCES[4]="- K&R C Programming Language
- Linux Socket Programming"
NEXT_WEEK[4]="Week05: 監視ツール基礎 + procfs 読解"

# Week05
GOALS[5]="監視ツール開発とprocfsの理解"
TODOS[5]="- [ ] CPU使用率の算出方法理解
- [ ] /proc/stat の解析
- [ ] CでCPU使用率取得プログラム作成
- [ ] Makefile 強化"
REFERENCES[5]="- procfs docs
- Linux Performance Book"
NEXT_WEEK[5]="Week06: ブログ週間（まとめ + 公開）"

# Week06 (Blog Week)
GOALS[6]="1〜5週のアウトプット整理とブログ公開"
TODOS[6]="- [ ] 技術ブログ執筆
- [ ] コード整理
- [ ] GitHub整理
- [ ] Phase1まとめ"
REFERENCES[6]="- Zenn / Qiita guidelines"
NEXT_WEEK[6]="Phase2 へ進む準備"

###############################################################################
# 2. Generation Logic
###############################################################################
if [[ -z "$1" ]]; then
    echo "Usage: ./generate_phase.sh phase01"
    exit 1
fi

PHASE="$1"
PHASE_DIR="$ISSUES_DIR/$PHASE"

mkdir -p "$PHASE_DIR"

for i in {1..6}; do
    WEEK="week$(printf "%02d" $i)"
    FILE="$PHASE_DIR/$WEEK.md"

    cat <<EOF >"$FILE"
# ${WEEK}

## 🎯 目標
${GOALS[$i]}

## ✅ ToDo（自動生成）
${TODOS[$i]}

## 📚 参考資料
${REFERENCES[$i]}

## 🔮 次週の予定
${NEXT_WEEK[$i]}
EOF

    echo "Generated: $FILE"
done

echo "\n[OK] Phase files generated in: $PHASE_DIR"
